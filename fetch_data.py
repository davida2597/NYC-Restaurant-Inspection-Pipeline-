...
def get_cursor_date() -> date:
    ...
    fallback = date.fromisoformat(FETCH_START_DATE)
    print(f"[cursor] Database empty or unreachable — falling back to {fallback} (fetching all data)")
    return fallback
...
def main():
    overall_start = get_cursor_date()
    print(overall_start, type(overall_start))
